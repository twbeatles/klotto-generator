...
- 현재 상태: 주요 항목 반영 완료
- 검증 상태: `pytest`, `pyright`, `compileall`, PyInstaller build 확인 예정/완료 기준으로 관리

## 반영된 항목

### 1. 동기화 / 데이터 무결성

- `estimate_latest_draw()`를 KST 기준 timezone-aware 계산으로 변경
...
- `scripts/export_to_excel.py` 흐름으로 최신 DB 상태를 엑셀 workbook으로 재생성 가능
